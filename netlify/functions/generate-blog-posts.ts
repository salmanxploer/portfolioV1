  ...
  try {
    ...
    if (action === "improve-post") {
      ...
      if (!String(improvePost.title || "").trim() || !String(improvePost.content || "").trim()) {
        return {
          statusCode: 400,
          headers: corsHeaders,
          body: JSON.stringify({ error: "Missing improvePost.title or improvePost.content for improve-post action." }),
        };
      }
      ...
      if (!improveCall.response.ok) {
        return {
          statusCode: improveCall.response.status,
          headers: corsHeaders,
          body: JSON.stringify({
            error: improveCall.data.error?.message || "Failed to improve post.",
          }),
        };
      }
      ...
      if (!improved) {
        return {
          statusCode: 422,
          headers: corsHeaders,
          body: JSON.stringify({ error: "AI returned an invalid improved post payload." }),
        };
      }
      ...
      return {
        statusCode: 200,
        headers: corsHeaders,
        body: JSON.stringify({ post: improved }),
      };
    }
    ...
    if (posts.length === 0) {
      return {
        statusCode: 422,
        headers: corsHeaders,
        body: JSON.stringify({ error: "AI returned an invalid posts payload." }),
      };
    }
